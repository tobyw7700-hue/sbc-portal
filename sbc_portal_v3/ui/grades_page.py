...
class GradesDashboard(tk.Frame):
    ...
    def _refresh(self):
        ...
        # Overall average (only subjects with grades)
        graded_raws = [g for _, g, _ in processed if g is not None]
        overall = round(sum(graded_raws) / len(graded_raws), 1) if graded_raws else None
        ...
        metric("GRADED",   str(len(graded_raws)), ACCENT)
        metric("PENDING",  str(len(subjects) - len(graded_raws)), FG_MUTED)
