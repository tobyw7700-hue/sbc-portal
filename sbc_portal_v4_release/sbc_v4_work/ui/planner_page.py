    ...
    def _auto_generate(self):
        ...
        setting_row("Subjects per day",
                    ...
                    lambda f: ttk.Combobox(f, textvariable=subjects_day_var,
                                           values=[1,2,3,4,5,6], state="readonly", width=6))
        setting_row("Study starts at",
                    "What time after school study sessions begin",
        ...
        tk.Label(f_we, text="Study on weekends", bg=BG_DARK, fg=FG_SEC,
                 font=FONT_BODY).pack(side="left", padx=8)
        tk.Label(f_we, text="Hours:", bg=BG_DARK, fg=FG_MUTED,
                 font=FONT_SMALL).pack(side="left", padx=4)
        ttk.Combobox(f_we, textvariable=weekend_h_var,
                     values=[1,2,3,4,5,6,7,8], state="readonly", width=4).pack(side="left")
        ...
        def generate():
            ...
            for i, dp in enumerate(self.day_plans):
                ...
                # Sleep (always)
                sleep_mins = (_time_to_mins(wake) + 24*60 - _time_to_mins(bed)) % (24*60)
                dp.blocks.append(Block("sleep", "Sleep", bed, sleep_mins))

                # Meals
                if meals_var.get():
                    for lbl, t, d in [("Breakfast","07:15",20),("Lunch","13:00",30),("Dinner","18:30",30)]:
                ...
                if not is_we or weekend_study_var.get():
                    total_study = (study_h if not is_we else int(weekend_h_var.get())) * 60
                    start_m = _time_to_mins(st_start if not is_we else "10:00")
                    # Pick which subjects to study today (cycle based on day index)
                    day_subjects = []
                    for j in range(subs_day):
                ...
                # Free time
                if free_var.get():
                    dp.blocks.append(Block("free","Free Time","20:30",60))
