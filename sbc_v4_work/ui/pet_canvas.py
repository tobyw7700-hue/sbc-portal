...
def draw_pet(canvas: tk.Canvas, species: str, level: int, cx: int, cy: int, size: int = 100):
    """Draw a pet centred at (cx, cy) with given size. Tier changes every 10 levels (0-9)."""
    cols = SPECIES_COLOURS.get(species, SPECIES_COLOURS["cat"])
    tier = min(level // 10, 9)   # 0-9, changes at levels 10,20,30...90
...
def _draw_wolf(c, cols, cx, cy, s, tier):
    ...
    if tier >= 3:
        aura = "#f0c040" if tier < 6 else "#c77dff" if tier < 8 else "#ffffff"
        _oval(c, cx, cy-5*s, (50+tier*3)*s, (45+tier*3)*s, "", aura, 1)
    if tier >= 6:
        # Glowing eyes
        _oval(c, cx-12*s, cy-22*s, 9*s, 9*s, "#ff6b35" if tier < 9 else "#ffffff", "", 0)
        _oval(c, cx+12*s, cy-22*s, 9*s, 9*s, "#ff6b35" if tier < 9 else "#ffffff", "", 0)
    if tier >= 8:
        # Divine sparkles around pet
        import math
        for angle in range(0, 360, 45):
            rad = math.radians(angle)
            sx = cx + math.cos(rad)*45*s
            sy = cy + math.sin(rad)*45*s
            _draw_star(c, sx, sy, 4*s, "#ffffff")
...
def _cloth_aurora_mantle(c, cx, cy, s):
    """Divine cape — shimmering aurora colours flowing behind the pet."""
    # Main flowing mantle
    _poly(c, [cx-18*s,cy-22*s, cx-65*s,cy,  cx-60*s,cy+60*s,
              cx+60*s,cy+60*s, cx+65*s,cy,   cx+18*s,cy-22*s],
          "#0d1b4a", "#0a1030", 2)
    # Aurora shimmer bands
    for i, (col, off) in enumerate([("#4af0c0",0),("#a855f7",6),("#3b82f6",12)]):
        _poly(c, [cx-60*s,cy+10*s+off*s, cx-55*s,cy+20*s+off*s,
                  cx+55*s,cy+20*s+off*s, cx+60*s,cy+10*s+off*s],
              col, "", 0)
    # Clasp
    _oval(c, cx, cy-18*s, 6*s, 6*s, "#ffffff", "#c0d8ff", 2)


def _cloth_storm_crown(c, cx, cy, s):
    """Mythic crown — dark storm cloud with lightning bolts."""
    hy = cy - 46*s
    # Cloud base
    _oval(c, cx, hy+6*s, 24*s, 10*s, "#334155", "#1e293b", 2)
    _oval(c, cx-14*s, hy+2*s, 12*s, 10*s, "#475569", "#334155", 1)
    _oval(c, cx+14*s, hy+2*s, 12*s, 10*s, "#475569", "#334155", 1)
    _oval(c, cx, hy-2*s, 16*s, 10*s, "#64748b", "#475569", 1)
    # Lightning bolts
    for dx in [-10, 0, 10]:
        pts = [cx+dx*s,hy+4*s, cx+(dx-4)*s,hy+14*s,
               cx+dx*s,hy+12*s, cx+(dx-6)*s,hy+24*s]
        c.create_line(*pts, fill="#fde047", width=max(1,int(2*s)))

# Items drawn BEHIND the pet (wings, capes, cloaks)
BACK_ITEMS = {
    "dragon_wings", "angel_wings", "black_cape", "cape",
    "shadow_cloak", "celestial_wings", "dragon_soul",
    "phoenix_feather", "aurora_mantle",
}

def draw_clothing(canvas: tk.Canvas, item_key: str, cx: int, cy: int,
                  size: int = 100, layer: str = "front"):
    """Draw clothing item. layer='back' draws behind pet, 'front' draws over pet."""
    if layer == "back" and item_key not in BACK_ITEMS:
        return
    if layer == "front" and item_key in BACK_ITEMS:
        return
    s = size / 100
    fns = {
        "crown":          _cloth_crown,
        ...
        # New items from v4 pet_models — mapped to closest existing draw fn
        "divine_halo":    lambda c,cx,cy,s: _cloth_halo(c,cx,cy,s),
        "void_crown":     lambda c,cx,cy,s: _cloth_crown(c,cx,cy,s),
        "celestial_wings":lambda c,cx,cy,s: _cloth_angel_wings(c,cx,cy,s),
        "gods_crown":     lambda c,cx,cy,s: _cloth_crown(c,cx,cy,s),
        "inferno_crown":  lambda c,cx,cy,s: _cloth_thunder_crown(c,cx,cy,s),
        "shadow_cloak":   lambda c,cx,cy,s: _cloth_black_cape(c,cx,cy,s),
        "cosmic_helm":    lambda c,cx,cy,s: _cloth_space_helmet(c,cx,cy,s),
        "dragon_soul":    lambda c,cx,cy,s: _cloth_dragon_wings(c,cx,cy,s),
        "eternity_ring":  lambda c,cx,cy,s: _cloth_laurel_wreath(c,cx,cy,s),
        "void_mask":      lambda c,cx,cy,s: _cloth_ninja_mask(c,cx,cy,s),
        "phoenix_feather":_cloth_phoenix_feather,
        "unicorn_horn":   _cloth_unicorn_horn,
        "samurai_helm":   _cloth_samurai_helm,
        "space_helmet":   _cloth_space_helmet,
        ...
        "paper_crown":    _cloth_paper_crown,
        "sweatband":      _cloth_sweatband,
        "wristband":      lambda c,cx,cy,s: _cloth_sweatband(c,cx,cy,s),
        "necklace":       lambda c,cx,cy,s: _cloth_collar(c,cx,cy,s),
        "friendship_band":lambda c,cx,cy,s: _cloth_sweatband(c,cx,cy,s),
        "sticker_star":   lambda c,cx,cy,s: _cloth_bow(c,cx,cy,s),
        "sticker_heart":  lambda c,cx,cy,s: _cloth_bow(c,cx,cy,s),
        "medal":          lambda c,cx,cy,s: _cloth_collar(c,cx,cy,s),
        "tag":            lambda c,cx,cy,s: _cloth_collar(c,cx,cy,s),
        "rubber_duck":    lambda c,cx,cy,s: _cloth_bow(c,cx,cy,s),
        "leaf":           lambda c,cx,cy,s: _cloth_flower_crown(c,cx,cy,s),
        "star_badge":     lambda c,cx,cy,s: _cloth_bow(c,cx,cy,s),
        "cloud_hat":      lambda c,cx,cy,s: _cloth_beanie(c,cx,cy,s),
        "bow_pink":       lambda c,cx,cy,s: _cloth_bow(c,cx,cy,s),
        "rainbow_hat":    _cloth_rainbow_hat,
        "elf_hat":        _cloth_elf_hat,
        "nurse_cap":      lambda c,cx,cy,s: _cloth_chef_hat(c,cx,cy,s),
        "pirate_eyepatch":_cloth_pirate_eyepatch,
        ...
        "mushroom_hat":   _cloth_mushroom_hat,
        "mortarboard":    _cloth_mortarboard,
        "aurora_mantle":  _cloth_aurora_mantle,
        "storm_crown":    _cloth_storm_crown,
        "celestial_diadem": lambda c,cx,cy,s: _cloth_crown(c,cx,cy,s),
    }
    fn = fns.get(item_key)
    if fn:
        fn(canvas, cx, cy, s)
...
def _draw_cat(c, cols, cx, cy, s, tier):
    # Tier 3+: aura glow behind
    if tier >= 3:
        aura = "#f0c040" if tier < 6 else "#c77dff" if tier < 8 else "#ffffff"
        for r in [52,46,40]:
            _oval(c, cx, cy-10*s, r*s, r*s, "", aura, 1)
    # Body — gets larger and more ornate at higher tiers
    body_w = (38 + tier*1.5)*s
    _oval(c, cx, cy+20*s, body_w, 32*s, cols["body"], cols["accent"], 2)
    # Head
    _oval(c, cx, cy-20*s, 35*s, 30*s, cols["body"], cols["accent"], 2)
    # Ears — gain inner detail at tier 2+
    ...
    # Eyes — glow at tier 4+
    eye_col = cols["eye"]
    if tier >= 4: eye_col = "#f0c040"
    if tier >= 7: eye_col = "#ff6b35"
    if tier >= 9: eye_col = "#ffffff"
    _oval(c, cx-12*s, cy-22*s, 7*s, 7*s, eye_col, "", 0)
    _oval(c, cx+12*s, cy-22*s, 7*s, 7*s, eye_col, "", 0)
    ...
    # Tail — appears at tier 1, gains extra curl at tier 5+
    if tier >= 1:
        c.create_arc(cx+20*s, cy+10*s, cx+60*s, cy+50*s,
                     start=90, extent=180, style="arc",
                     outline=cols["accent"], width=max(2,int(4*s)))
    if tier >= 5:
        c.create_arc(cx+35*s, cy-5*s, cx+65*s, cy+25*s,
                     start=270, extent=180, style="arc",
                     outline=cols["nose"], width=max(2,int(3*s)))
    # Tier 2+: markings on body
    if tier >= 2:
        for dy in [-5, 5]:
            c.create_line(cx-15*s, cy+dy*s+20*s, cx+15*s, cy+dy*s+20*s,
                          fill=cols["accent"], width=max(1,int(2*s)))
    # Tier 6+: crown-like head markings
    if tier >= 6:
        for dx in [-12, 0, 12]:
            _poly(c, [cx+dx*s-4*s, cy-46*s, cx+dx*s, cy-56*s, cx+dx*s+4*s, cy-46*s],
                  "#f0c040", "#c09020", 1)
    # Paws
    _oval(c, cx-22*s, cy+48*s, 12*s, 8*s, cols["body"], cols["accent"], 1)
    _oval(c, cx+22*s, cy+48*s, 12*s, 8*s, cols["body"], cols["accent"], 1)
...
class PetWidget(tk.Frame):
    ...
    def draw(self):
        ...
        # Draw wings/capes BEHIND the pet
        if self.equipped_item:
            draw_clothing(self.canvas, self.equipped_item, cx, cy, self.size, layer="back")
        draw_pet(self.canvas, self.species, self.level, cx, cy, self.size)
        # Draw hats/accessories OVER the pet
        if self.equipped_item:
            draw_clothing(self.canvas, self.equipped_item, cx, cy, self.size, layer="front")
